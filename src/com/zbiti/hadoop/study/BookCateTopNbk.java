...
public class BookCateTopNbk extends Configured implements Tool {
	...
	private static class JoinReducer extends Reducer<Text, Text, Text, Text> {
		...
		protected void reduce(Text key, Iterable<Text> values, Context context)
			...
			if (key.toString().contains("topca#")) {
				for (Text count : values) {
					treeMap.put(keys[1], Integer.valueOf(count.toString()));
				}
			} else {
		...
		protected void cleanup(Reducer<Text, Text, Text, Text>.Context context)
			...
			while (titer.hasNext()) {
				...
				String name = cate.get(key);
				context.write(new Text(key), new Text(val + "\t" + name));
			}
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new BookCateTopNbk(), args);
		System.exit(res);
	...
	public int run(String[] args) throws Exception {
		...
		try {
			...
			jobGroup.setJarByClass(BookCateTopNbk.class);
			...
			if (jobGroup.waitForCompletion(true)) {
				...
				jobOrder.setJarByClass(BookCateTopNbk.class);
